...
import { SizableText, XStack, YStack, isWeb } from 'tamagui'
...
// isWeb: window.location doesn't exist on react-native — native navigation
// needs one's router (internal) + Linking (external) instead
...
export function WikiStartpage() {
  // the startpage is keyboard-driven (chords, '/' focus, raw form/input) — a
  // native version needs its own design, not a port. web-gate the whole thing.
  if (!isWeb) return null
  return <WikiStartpageWeb />
}

function WikiStartpageWeb() {
  ...
  useEffect(() => {
    // isWeb: document / window key listeners below don't exist on react-native
    if (!isWeb) return
